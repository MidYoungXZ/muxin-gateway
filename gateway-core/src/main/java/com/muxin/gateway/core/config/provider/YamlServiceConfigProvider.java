...
@Slf4j
public class YamlServiceConfigProvider implements ServiceConfigProvider {
    ...
    private final List<ServiceDefinition> services = new CopyOnWriteArrayList<>();
    ...
    public YamlServiceConfigProvider() {
        this("gateway-routes.yml");
    }

    public YamlServiceConfigProvider(String configFile) {
    ...
    private void loadConfig() {
        try {
            ...
            List<ServiceDefinition> newServices = config.getServices() != null ? config.getServices() : List.of();
            services.clear();
            services.addAll(newServices);
            if (log.isDebugEnabled()) {
                log.debug("Loaded {} services from YAML config", services.size());
            }
        } catch (Exception e) {
            log.error("Failed to load service config from YAML: {}", configFile, e);
        }
    }
    ...
    public void refresh() {
        if (log.isInfoEnabled()) {
            log.info("Refreshing service configuration from YAML: {}", configFile);
        }
        loadConfig();
        List<String> ids = services.stream().map(ServiceDefinition::getId).toList();
    ...
    public void startWatching() {
        ...
        try {
            ...
            watchExecutor = Executors.newSingleThreadExecutor(r -> {
                Thread t = new Thread(r, "yaml-service-config-watcher");
                t.setDaemon(true);
                return t;
            });
            ...
            if (log.isInfoEnabled()) {
                log.info("Started watching config file for services: {}", configPath);
            }
        } catch (IOException e) {
    ...
    public void stopWatching() {
        ...
        if (log.isInfoEnabled()) {
            log.info("Stopped watching config file for services");
        }
    }
            ...
            try {
                ...
                for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    if (configFile.equals(changedFile.toString())) {
                        if (log.isInfoEnabled()) {
                            log.info("Config file changed, refreshing services: {}", changedFile);
                        }
                        Thread.sleep(100);
                        refresh();
                    }
