...
graph TB
    ...
    subgraph RouteLayer["🚦 路由层"]
        ...
        EnhancedRouteManager[EnhancedRouteManager<br/>增强路由管理器 ✅]
        RouteMatcher[RouteMatcher<br/>路由匹配器 ✅]
        RouteConfigConverter[RouteConfigConverter<br/>路由配置转换器 ✅]
        GlobalRouteConfig[GlobalRouteConfig<br/>全局路由配置 ✅]
    end
    ...
    subgraph FilterLayer["🔧 过滤器层"]
        ...
        FilterDefinition[FilterDefinition<br/>过滤器定义 ✅]
    ...
    subgraph LoadBalanceLayer["⚖️ 负载均衡层"]
        ...
        LoadBalanceDefinition[LoadBalanceDefinition<br/>负载均衡定义 ✅]
...
#### 🚀 架构简化（2025-01-20重构）
**重构背景**: 为了简化架构，减少不必要的抽象层，提高系统的可维护性和性能。

**主要变更**:
- ❌ **移除Manager抽象层**: 删除了LoadBalanceManager、FilterManager、PredicateManager接口
- ✅ **功能集成**: 将负载均衡和过滤器管理功能集成到RouteConfigConverter中
- ✅ **增强路由管理**: EnhancedRouteManager集成了所有Manager功能，支持全局配置
- ✅ **全局配置支持**: 新增GlobalRouteConfig，支持全局默认配置和继承机制
- ✅ **完全隔离**: 每个路由独立创建实例，避免配置冲突
- ✅ **动态更新**: 支持重新创建整个Route对象的简单更新策略

**架构简化前后对比**:
```
简化前: GatewayProcessor → FilterManager → Filter实例
                        → LoadBalanceManager → LoadBalancer实例  
                        → PredicateManager → Predicate实例

简化后: GatewayProcessor → EnhancedRouteManager (集成了所有Manager功能)
                        → RouteConfigConverter (Factory模式创建组件)
                        → GlobalRouteConfig (全局配置管理)
```
...
- **策略模式**: 负载均衡和过滤器策略
- **责任链模式**: 过滤器链和断言链
- **工厂模式**: RouteConfigConverter统一创建组件 ✅（强化）
- **观察者模式**: 事件监听和通知
- **组合模式**: GlobalRouteConfig合并路由配置 ✅（新增）
